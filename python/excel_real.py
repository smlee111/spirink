...
conn = pymysql.connect(host='192.168.180.236', user='playcom', password='tmvlfldzm!23',
                       db='spreal', port=39306, charset='utf8')
#데이터 입력
table_name = 'tb_work_'+bd_idx
curs = conn.cursor()
sql = "insert into {0}(uni_num,bd_idx,quest,answer) values (%s, %s, %s, %s)".format(table_name)

# Iterate through each row in worksheet and fetch values into dict
for row in islice(sheet.values, 1, sheet.max_row):
    print(row[0])
    curs.execute(sql, (row[0],bd_idx,row[1],row[2]))
#마스터 업데이트
