...
select_sql = "SELECT wk_idx, answer,sp_answer FROM {0}".format(table_name)
chk_upd_sql = "update {0} set sp_quest=%s,mod_contents=%s, sp_answer=%s where wk_idx=%s".format(table_name)
#print("select....start")
...
for x in mr:
    ...
    if result_tot != quest:
        ...
        if not sp_answer:
            ...
            sp_answer=sp_answer+",CO"
    #result2 = result_tot.replace("$$","\n")
    #quest = quest.replace("$$","\n")
    #mod_db =  mod_str.replace("$$","</br>")
    ...
    mc.execute(chk_upd_sql, (result2,mod_db,sp_answer,wk_idx))
