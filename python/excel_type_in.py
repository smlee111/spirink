...
curs = conn.cursor()
sql = "insert into {0}(uni_num,bd_idx,worker_nm,noun_list,quest) values (%s, %s, %s, %s, %s)".format(table_name)

# Iterate through each row in worksheet and fetch values into dict
for row in islice(sheet.values, 3, sheet.max_row):
    print(row[0],row[1],row[2],row[3])
    curs.execute(sql, (row[0],bd_idx,row[1],row[2],row[3]))
#마스터 업데이트
