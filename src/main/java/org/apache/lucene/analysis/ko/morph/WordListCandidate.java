...
public class WordListCandidate implements Cloneable {
  ...
  public int getCorrectLength() {
    ...
    for(List<AnalysisOutput> outputs : wordList) {
      ...
      //System.out.println("outputs.get(0).getScore()==>"+outputs.toString()+outputs.get(0).getScore());
  ...
  //체언+조사, 부사+조사, 단일어 갯수파악
  public int getPtnCount() {	    
    if(this.ptbnCount!=-1) return ptbnCount;
    ptbnCount = 0;
    ...
    for(List<AnalysisOutput> outputs : wordList) {
      ...
      if(ptn==PatternConstants.PTN_NJ || ptn <= PatternConstants.PTN_AID ||ptn <= PatternConstants.PTN_ADVJ) {
    	  ptbnCount++;
      }
    }
