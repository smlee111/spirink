...
public final class SpKoreanFilter extends TokenFilter {
  ...
  public SpKoreanFilter(TokenStream input) {
    this(input, true);
  }
   ...
   */
  public SpKoreanFilter(TokenStream input, boolean bigram) {
    this(input, bigram, false);
  }
  
  public SpKoreanFilter(TokenStream input, boolean bigram, boolean has) {
    this(input, bigram, has, false);
  }
  
  public SpKoreanFilter(TokenStream input, boolean bigram, boolean has, boolean exactMatch) {
    this(input, bigram, has, exactMatch, true);
  }

  public SpKoreanFilter(TokenStream input, boolean bigram, boolean has, boolean exactMatch, boolean cnoun) {
  ...
  public boolean incrementToken() throws IOException {
    ...
    /*for(int i=0;i<morphQueue.size();i++) {
    	System.out.println(morphQueue.get(i).getOutputs().toString());
    }*/
  ...
  private void setAttributesFromQueue(boolean isFirst) {
    ...
    System.out.println("===>"+morphAtt.getToken().getTerm().toString());
 ...
 * @throws MorphException 
  ...
  private void analysisKorean(String input) throws MorphException {
    ...
    for(int i=0;i<map.size();i++) {
    	KoreanToken t = map.get(i);
    	if(t!=null)
    	System.out.println("t.getOutputs()"+t.getOutputs());
    }
