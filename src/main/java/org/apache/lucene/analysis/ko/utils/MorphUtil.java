...
public class MorphUtil {
  ...
  //음소의 위치를 찾는다.
  public static int searchPosition(int position, char source){
	  int i=-1;
	  if(position==1){
		  i= Arrays.binarySearch(MorphUtil.CHOSEONG, source);
	  }else if(position==2){
		  i= Arrays.binarySearch(MorphUtil.JUNGSEONG, source);
	  }else{
		  i= Arrays.binarySearch(MorphUtil.JONGSEONG, source);
	  }
	  return i;
  }
  ...
  public static void main(String args[]){
	  int chk = Arrays.binarySearch(CHOSEONG, 'ㅃ');
