...
public class EomiUtil {
  ...
  private static final char[] CHK_ARRAY= {'다','어'}; //어미의 종류 체크한다
  ...
  public static String[] splitPomi(String stem) throws MorphException  {
    ...
    if(nChrs.length==2&&chrs[index]=='시'&&(chrs.length<=index+1||
      ...
      if(DictionaryUtil.getVerb(results[0])!=null) return results;  //'시'가 포함된 단어가 있다. 성가시다/도시다/들쑤시다 
      pomi = chrs[index]+pomi;  
      setPomiResult(results,stem.substring(0,index),pomi);      
  ...
  public static String makeTesnseEomi(String preword, String endword) {
   ...
   public static boolean endsWithEEomi(String stem) {
     ...
     char[] jasos = MorphUtil.decompose(stem.charAt(len-2));//한글한글자를 초성중성종성배열로 반환한다.
     for(int i=0;i<jasos.length;i++){
    	 System.out.println(i+" "+jasos[i]);
     }
   ...
   /**
    * 자음+으어 인지 조사한다. 쁘(V),어(e)
    */
  public static boolean endsWithUEEomi(String stem) {
     int len = stem.length();
     //if(!chkArray(stem)) return false;
     
     char[] jasos = MorphUtil.decompose(stem.charAt(len-2));//한글한글자를 초성중성종성배열로 반환한다.
     for(int i=0;i<jasos.length;i++){
    	 System.out.println(i+" "+jasos[i]);
     }
     if(jasos.length==2&&jasos[1]=='ㅡ')
       return true;
     else {
       return false;
     }
  ...
  public static boolean chkArray(String stem){
	  boolean chk=false;
	  int len = stem.length();
	  
	  int position = Arrays.binarySearch(CHK_ARRAY, '다');
	  System.out.println("===>"+stem.charAt(len-1)+"position"+position);
	  if(len>1&&position>-1)chk =true;
	  return chk;
  }
  /**
   * 쁘(V),어(e) ->쁨으로 바꾼다
   */
 public static String tranUEEomi(String stem) {
    String str="";
    char ch;;
	int len = stem.length();
    if(len<2){
    	...
    	char[] jasos = MorphUtil.decompose(stem.charAt(len-2));//한글한글자를 초성중성종성배열로 반환한다.
    	if(jasos.length==2&&jasos[1]=='ㅡ'){
    		int first = MorphUtil.searchPosition(1, jasos[0]);
    		int jungsung = MorphUtil.searchPosition(2, 'ㅡ');
    		int jongsung = MorphUtil.searchPosition(3, 'ㅁ');
    		ch = MorphUtil.compound(first,jungsung,jongsung);
    		str = stem.substring(0,len-2)+String.valueOf(ch);
        }else {
    		str = stem.substring(0,len-1)+"음";
    	}
    }
    return str;
 }
 ...
 /**
  * 쁘(V),었(f),다(e) ->뻤음
  * anycont=보(V),았(f),다(e) -> 봤음
  */
public static String tranUEPassEomi(String stem) {
   String str="";
   char ch;;
	int len = stem.length();
   if(len<3){
   	...
   	char[] jasos1 = MorphUtil.decompose(stem.charAt(len-3));//쁘를 나눈다
   	char[] jasos2 = MorphUtil.decompose(stem.charAt(len-2));//어를 나눈다
   	System.out.println("jasos1"+jasos1[0]+jasos1[1]);
   	System.out.println("jasos2"+jasos2[0]+jasos2[1]+jasos2[2]);
   	if(jasos1[1]=='ㅡ'){
	   	int first = MorphUtil.searchPosition(1, jasos1[0]);
	   	int jungsung = MorphUtil.searchPosition(2, jasos2[1]);
	   	int jongsung = MorphUtil.searchPosition(3, jasos2[2]);
	   	ch = MorphUtil.compound(first,jungsung,jongsung);
		str = stem.substring(0,len-3)+String.valueOf(ch)+"음";
   	}else if(jasos1[1]=='ㅗ' && jasos2[1]=='ㅏ'){
	   	int first = MorphUtil.searchPosition(1, jasos1[0]);
	   	int jungsung = MorphUtil.searchPosition(2, 'ㅘ');
	   	int jongsung = MorphUtil.searchPosition(3, jasos2[2]);
	   	ch = MorphUtil.compound(first,jungsung,jongsung);
		str = stem.substring(0,len-3)+String.valueOf(ch)+"음";
   	}else if(jasos2[2]=='ㅆ'){
...
/**
 * 
 * anycont=보(V),았(f),다(e) -> 봤어
 */
public static String tranVfeHeEomi(String stem) {
  String str="";
  char ch;;
	int len = stem.length();
  if(len<3){
  	...
  	if(jasos1[1]=='ㅡ'){
	   	...
	   	int jongsung = MorphUtil.searchPosition(3, jasos2[2]);
	   	ch = MorphUtil.compound(first,jungsung,jongsung);
		str = stem.substring(0,len-3)+String.valueOf(ch)+"어";
  	}else if(jasos1[1]=='ㅗ' && jasos2[1]=='ㅏ'){
	   	int first = MorphUtil.searchPosition(1, jasos1[0]);
	   	int jungsung = MorphUtil.searchPosition(2, 'ㅘ');
	   	int jongsung = MorphUtil.searchPosition(3, jasos2[2]);
...
/**
 * 
 * anycont=보(V),았(f),다(e) -> 봤어요
 */
public static String tranVfeHeYEomi(String v,String f,String e,boolean inGative) {
  String str="";
  int vLen = v.length();
  char ch;
  	char[] jasos1 = MorphUtil.decompose(v.charAt(vLen-1));//쁘 를 나눈다
  	char[] jasos2 = MorphUtil.decompose(f.charAt(0));//었 을 나눈다
  	System.out.println("jasos1"+jasos1[0]+jasos1[1]);
  	System.out.println("jasos2"+jasos2[0]+jasos2[1]+jasos2[2]);
  	if(jasos1[1]=='ㅡ'){
	   	int first = MorphUtil.searchPosition(1, jasos1[0]);
		...
		str = v.substring(0,v.length()-1)+String.valueOf(ch)+"어요";
  	}else if(jasos1[1]=='ㅗ' && jasos2[1]=='ㅏ'){
	   	int first = MorphUtil.searchPosition(1, jasos1[0]);
	   	int jungsung = MorphUtil.searchPosition(2, 'ㅘ');
	   	int jongsung = MorphUtil.searchPosition(3, jasos2[2]);
...
/**
 * 
 * {stype=N, sourceChar=생일}, {stype=t, sourceChar=이}, {stype=f, sourceChar=었}, {stype=e, sourceChar=다}
 *  -> 봤어요
 */
public static String tranNtfeHeYEomi(String n,String t,String f,String e) {
  String str="";
  char ch;
  	char[] jasos1 = MorphUtil.decompose(f.charAt(0));//었을 나눈다
  	char[] jasos2 = MorphUtil.decompose(e.charAt(0));//다를 나눈다
  	System.out.println("jasos1"+jasos1[0]+jasos1[1]);
  	...
  	if(jasos1[1]=='ㅡ'){
		...
		System.out.println("1"+str);
	   	...
	   	int jongsung = MorphUtil.searchPosition(3, jasos2[2]);
	   	ch = MorphUtil.compound(first,jungsung,jongsung);
		str = n+t+String.valueOf(ch)+"어요";
		System.out.println("2"+str);
  	}else if(jasos1.length==3&& jasos1[2]=='ㅆ'){
	   	//int first = MorphUtil.searchPosition(1, jasos1[0]);
	   	//int jungsung = MorphUtil.searchPosition(2, 'ㅘ');
	   	//int jongsung = MorphUtil.searchPosition(3, jasos2[2]);
	   	//ch = MorphUtil.compound(first,jungsung,jongsung);
  ...
  * 싶(V),네(e) ->싶어
  */
public static String tranVeHEEomi(String v,String e) {
   String str="";
   char ch;;
	int vLen = v.length();
   
   	char[] jasos = MorphUtil.decompose(v.charAt(vLen-1));//한글한글자를 초성중성종성배열로 반환한다.
   	if(jasos.length==2) {
   	  if(jasos[1]=='ㅡ'){
   		int first = MorphUtil.searchPosition(1, jasos[0]);
   		int jungsung = MorphUtil.searchPosition(2, 'ㅓ');
   		int jongsung = 0;
   		ch = MorphUtil.compound(first,jungsung,jongsung);
   		str = v.substring(0,vLen-1)+String.valueOf(ch);
       }
   	}else if(jasos.length==3) {
	  if(jasos[1]=='ㅏ'){
   		/*int first = MorphUtil.searchPosition(1, jasos[0]);
   		int jungsung = MorphUtil.searchPosition(2, 'ㅏ');
   		int jongsung = 0;
   		ch = MorphUtil.compound(first,jungsung,jongsung);*/
   		str = v+"아";
       }else if(jasos[1]=='ㅣ'||jasos[1]=='ㅓ'){
      		/*int first = MorphUtil.searchPosition(1, jasos[0]);
       		int jungsung = MorphUtil.searchPosition(2, 'ㅓ');
       		int jongsung = 0;
       		ch = MorphUtil.compound(first,jungsung,jongsung);*/
   ...
   return str;
	 ...
	 * 쁘(V),어(e) ->쁩니다로 바꾼다
	 */
	public static String tranHAPEomi(String stem,boolean inGative) {
	  String str="";
	  String comemi="니다";
	  if(inGative) comemi="니까";
	  char ch;;
		int len = stem.length();
	  if(len<2){
	  	str = stem;
	  	...
	  	if(jasos.length==2&&jasos[1]=='ㅡ'){
	  		int first = MorphUtil.searchPosition(1, jasos[0]);
	  		int jungsung = MorphUtil.searchPosition(2, 'ㅡ');
	  		int jongsung = MorphUtil.searchPosition(3, 'ㅂ');
	  		ch = MorphUtil.compound(first,jungsung,jongsung);
	  		str = stem.substring(0,len-2)+String.valueOf(ch+comemi);
	     }else if(jasos.length==3&&jasos[2]=='ㅆ'){
		  		//int first = MorphUtil.searchPosition(1, jasos[0]);
		  		//int jungsung = MorphUtil.searchPosition(2, 'ㅡ');
		  		//int jongsung = MorphUtil.searchPosition(3, 'ㅂ');
		  		//ch = MorphUtil.compound(first,jungsung,jongsung);
		  		str = stem.substring(0,len-1)+"습"+comemi;
		 }else {
			 str = stem.substring(0,len-1)+"습"+comemi;
	  	 }
	  }
	...
	/**
	 * 
	 * anycont=보(V),았(f),다(e) -> 봤습니다 , 예쁘(V),었(f),음(e) -> 예뻤습니다
	 *  먹었어->먹었습니다   자음여부에 따라서 없으면 합치고 있으면 해당자음그대로 유지
 	 */
	public static String tranVfeHapEomi(String v,String f,String e, boolean inGative) {
	  String str="";
	  String comemi="니다";
	  if(inGative) comemi="니까";
	  char ch;;
		int vLen = v.length();
	  	...
	  	char[] jasos1 = MorphUtil.decompose(v.charAt(vLen-1));//쁘를 나눈다
	  	char[] jasos2 = MorphUtil.decompose(f.charAt(0));//었를 나눈다
	  	System.out.println("jasos1"+jasos1[0]+jasos1[1]);
	  	System.out.println("jasos2"+jasos2[0]+jasos2[1]+jasos2[2]);
	  	if(jasos1.length==2) {//자음이 없는경우
		  	if(jasos1[1]=='ㅡ'){
			   	int first = MorphUtil.searchPosition(1, jasos1[0]);
			   	int jungsung = MorphUtil.searchPosition(2, jasos2[1]);
			   	int jongsung = MorphUtil.searchPosition(3, jasos2[2]);
			   	ch = MorphUtil.compound(first,jungsung,jongsung);
				str = v.substring(0,vLen-1)+String.valueOf(ch)+"습"+comemi;
		  	}else if(jasos1[1]=='ㅗ' && jasos2[1]=='ㅏ'){
			   	int first = MorphUtil.searchPosition(1, jasos1[0]);
			   	int jungsung = MorphUtil.searchPosition(2, 'ㅘ');
			   	int jongsung = MorphUtil.searchPosition(3, jasos2[2]);
	  	...
	  	}else if(jasos1.length==3) { //자음이 있는경우
	  		str = v+f+"습"+comemi;
	  	}
	 ...
	 *  -> 생일이었습니다.
	 */
	public static String tranNtfeHapEomi(String n,String t,String f,String e,boolean inGative) {
	  String str="";
	  String comemi="니다";
	  if(inGative) comemi="니까";
	  char ch;
		
	  	char[] jasos1 = MorphUtil.decompose(t.charAt(0));//이
	  	char[] jasos2 = MorphUtil.decompose(f.charAt(0));//었
	  	System.out.println("jasos1"+jasos1[0]+jasos1[1]);
	  	System.out.println("jasos2"+jasos2[0]+jasos2[1]+jasos2[2]);
	  	if(jasos1[1]=='ㅡ'){
		   	int first = MorphUtil.searchPosition(1, jasos1[0]);
		   	...
		   	int jongsung = MorphUtil.searchPosition(3, jasos2[2]);
		   	ch = MorphUtil.compound(first,jungsung,jongsung);
			str = n+t+String.valueOf(ch)+comemi;
	  	}else if(jasos1[1]=='ㅗ' && jasos2[1]=='ㅏ'){
		   	int first = MorphUtil.searchPosition(1, jasos1[0]);
		   	int jungsung = MorphUtil.searchPosition(2, 'ㅘ');
		   	int jongsung = MorphUtil.searchPosition(3, jasos2[2]);
		   	ch = MorphUtil.compound(first,jungsung,jongsung);
			str = n+t+String.valueOf(ch)+"습"+comemi;
	  	}else if(jasos2[2]=='ㅆ'){
		   	//int first = MorphUtil.searchPosition(1, jasos1[0]);
		   	//int jungsung = MorphUtil.searchPosition(2, 'ㅘ');
		   	//int jongsung = MorphUtil.searchPosition(3, jasos2[2]);
  ...
  public static void main(String args[]){
	  try {
		  boolean a = endsWithUEEomi("예쁘다");
		  System.out.println("a===>"+a);
		  

		  /*String[]  strArr = splitEomi("예뻐", "요");
		  ...
		  String stem ="예뻐";
		  String end ="요";
		  irregularStem(results, stem, end);
		  if(results.size()<1)
		  irregularEnding(results, stem, end);
		  if(results.size()<1)
		  irregularAO(results, stem, end);
